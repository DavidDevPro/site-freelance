...
const QuestionnairePage = () => {
  ...
  const steps = [
    {
      ...
      content: <Step1Formule />,
    },
    {
      title: "Étape 2 : Sélectionnez les options",
      content: <Step2Options addPages={addPages} setAddPages={setAddPages} pageCount={pageCount} setPageCount={setPageCount} />,
    },
    {
  ...
  return (
    <FormProvider {...methods}> {/* Envelopper avec FormProvider */}
      <div className="max-w-3xl mx-auto py-12 px-4">
        <div className="flex justify-center items-end max-w-screen-lg mx-auto mb-8">
          {steps.map((step, index) => (
            <div key={index} className={`w-full ${index < steps.length - 1 ? 'mr-8' : ''}`}>
              <h6 className={`text-base font-bold mb-2 text-center ${currentStep >= index ? 'text-primary' : 'text-muted-foreground'}`}>
                {step.title}
              </h6>
              <div className="flex items-center w-full">
                <div className={`w-8 h-8 shrink-0 mx-[-1px] ${currentStep >= index ? 'bg-primary' : 'bg-muted'} p-1.5 flex items-center justify-center rounded-full`}>
                  {currentStep >= index ? (
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-full fill-primary-foreground" viewBox="0 0 24 24">
                ...
                {index < steps.length - 1 && (
                  <div className={`w-full h-1 ${currentStep >= index ? 'bg-primary' : 'bg-muted'}`}></div>
                )}
              </div>
            </div>
          ))}
        ...
        <div className="flex justify-between">
          ...
          {currentStep < steps.length - 1 ? (
            ...
            <StyledButton variant="primary" onClick={() => alert('Formulaire soumis !')}>
              Soumettre
            </StyledButton>
          )}
        </div>
